...
- `custom_components/vault/` - Main integration code
...
- Integration logs: `custom_components.vault: debug` in `config/configuration.yaml`
...
- Use the domain `vault` for all DOMAIN references
- Prefix all integration-specific classes with `Vault`
- Use "Vault" as the display title
...
- Inherit from platform base + `VaultEntity`
...
- `tests/` mirrors `custom_components/vault/` structure
