...
- **`entity/`** - Base entity classes (inherit `VaultEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, VaultEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import VaultEntity`
